...
gulp.task('minifymanager', function(cb) {
...
gulp.task('minifycomponents', function(cb) {
...
gulp.task("online_replaceRequireConfPath",function (cb) {
    gulp.src(['rev-manifest.json', './dist/javascripts/manager/requireConf-*.js'])
        .pipe(revCollector())   //- 替换为MD5后的文件名
        .pipe(rename('requireConf.js')) //每次发布必更新的文件直接使用系统时间
        .pipe(gulp.dest('./dist/javascripts/manager/'))
        .on('end', cb);
});

gulp.task('online_cleanRequireConf', function(cb) {
    gulp.src(['./dist/javascripts/manager/requireConf-*.*'])
            .pipe(clean())
            .on('end', cb);
});
